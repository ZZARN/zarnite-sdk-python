...
class PlaygroundMetricsEvent(BaseModel):
    """
    PlaygroundMetricsEvent
    """ # noqa: E501
    id: StrictInt = Field(description="Event row ID")
    event_type: StrictStr = Field(description="Event type (stt_complete, tts_complete, etc.)", alias="eventType")
    payload: Optional[Dict[str, Any]] = Field(default=None, description="Event payload")
    created_at: StrictStr = Field(description="Event timestamp", alias="createdAt")
    __properties: ClassVar[List[str]] = ["id", "eventType", "payload", "createdAt"]
